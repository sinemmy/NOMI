import argparse
import time
import brainflow
import numpy as np

from brainflow.board_shim import BoardShim, BrainFlowInputParams, LogLevels, BoardIds
from brainflow.data_filter import DataFilter, FilterTypes, AggOperations, WindowFunctions, DetrendOperations


def main():
    BoardShim.enable_dev_board_logger()

    # use synthetic board for demo
    params = BrainFlowInputParams()
    board_id = -1
    board_descr = BoardShim.get_board_descr(board_id)
    params.serial_port = 'COM3'
    sampling_rate = int(board_descr['sampling_rate'])
    board = BoardShim(board_id, params)
    board.prepare_session()
    board.start_stream()
    BoardShim.log_message(LogLevels.LEVEL_INFO.value, 'start sleeping in the main thread')
    time.sleep(10)

    # board.stop_stream()
    # board.release_session()
    print('starting')
    timeout = time.time() + 60
    # eegCHAN = [1, 2, 3, 4, 5, 6, 7, 8]
    eeg_channels = board_descr['eeg_channels']
    # second eeg channel of synthetic board is a sine wave at 10Hz, should see huge alpha
    eeg_channel = eeg_channels[1]
    while True:
        if time.time() > timeout:
            break
        nfft = DataFilter.get_nearest_power_of_two(sampling_rate)

        data = board.get_current_board_data(4500)
        # if time.time() == secondTimer:
        for channel in eeg_channels:
            print(data[channel], channel-1)
            print(data[channel + 15], channel + 15)

            # DataFilter.detrend(data[channel], DetrendOperations.LINEAR.value)
            DataFilter.perform_rolling_filter(data[channel], 3, AggOperations.MEDIAN.value)
            DataFilter.perform_lowpass(data[channel], sampling_rate, 50.0, 5,
                                  FilterTypes.CHEBYSHEV_TYPE_1.value, 1)

            psd = DataFilter.get_psd_welch(data[channel], nfft, nfft // 2, sampling_rate,
                                           WindowFunctions.BLACKMAN_HARRIS.value)

            band_power_alpha = DataFilter.get_band_power(psd, 7.0, 13.0)
            band_power_beta = DataFilter.get_band_power(psd, 14.0, 30.0)
            #print("alpha/beta:%f", band_power_alpha / band_power_beta, "current channel: " + str(channel))

            # # # fail test if ratio is not smth we expect
            # if ((band_power_alpha / band_power_beta) > 100):
            #     raise ValueError('Wrong Ratio')
            # # secondTimer == time.time() + 10
        exit()
        continue






    # optional detrend



if __name__ == "__main__":
    main()